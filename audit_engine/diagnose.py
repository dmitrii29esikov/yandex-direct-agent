...
def diagnose(account: str, findings, target_cpa=None) -> list:
    ...
    if text_len:
        ...
        diagnoses.append(Diagnosis(
            ...
            contour="direct",
            summary=(
                f"{total_ads} объявлений в {len(text_len)} кампаниях превышают лимиты "
                f"Директа (заголовок 56, текст 81 символ). Такие объявления не "
                f"показываются, то есть часть групп работает вслепую."
            ),
