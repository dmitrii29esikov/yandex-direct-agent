...
@register("DIRECT.DEAD_GOALS", "direct", "warning",
          description="Цели, включённые в кампании Директа, не срабатывают")
def check_dead_goals(ctx):
    """
    Цель, включённая в кампанию Директа, за период не дала конверсий.

    Смысл: стратегия обучается на той цели, которая выбрана в кампании. Если
    цель молчит, автостратегия не получает сигнала, и бюджет расходуется без
    обучения.

    Важно: считаем только цели, задействованные в кампаниях Директа. В счётчике
    Метрики обычно есть и другие цели — по воронке и для аналитики; они не
    участвуют в закупке, и «мёртвыми» их считать нельзя.
    """
    ...
    return [dict(
        title="Цели в Директе не срабатывают",
        detail=f"За период ни разу не сработали цели, включённые в кампании "
               f"Директа: {len(dead)} из {len(matrix.get('goals') or [])}. Именно "
               f"на них обучаются стратегии: без сигнала автостратегия расходует "
               f"бюджет вслепую. Аналитические цели счётчика (в кампании не "
               f"включены) в этот список не попадают — на закупку они не влияют",
        object_type="account", object_id=ctx.account,
        evidence={"целей в Директе проверено": len(matrix.get("goals") or []),
                  "не сработали": len(dead), "примеры": examples},
        fix="Проверить в интерфейсе цель в стратегии кампании: если она не "
            "срабатывает — заменить на рабочую или починить саму цель в Метрике",
        fixable="manual",
    )]
